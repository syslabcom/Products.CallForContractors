...
def handle_object_translation(obj, event):
    """ copy exisitng references to the target.
    If a referenced object exits in the target's language, use it instead of the canonical's reference """
    obj = obj.getCanonical()
    # get all ReferenceFields 
    fields = [x for x in obj.Schema().fields() if isinstance(x, ReferenceField)]
    # only handle fields which are not language independent - the others are already taken care of
    # by LinguaPlone
    ...
    for field in fields:
        ...
        refobjtrans = getattr(event.target, transname, getattr(obj, transname, None))
...
def handle_object_uploaded(event):
    """ If a file was uploaded, check if it is a translated version of an already referenced 
    file on the canonical. In that case, set the correct reference on the translated CallForContractors. """
    obj = event.object
    ...
    if ICallForContractors.providedBy(parent) and IATFile.providedBy(obj):
        ...
        if lang != default_lang:
            ...
            if transobj:
                fields = [x for x in canparent.Schema().fields() if isinstance(x, ReferenceField)]
                fields = [x for x in fields if not x.languageIndependent]
                for field in fields:
                    ...
                    if refobj is None or type(refobj) == type(list()):
                        continue
                    fname = refobj.getFilename()
                    langtrans, namestemtrans, suffixtrans = _guessLanguage(refobj, fname)
                    # is the uploaded file a translated version of a already referenced file?
                    if namestem == namestemtrans:
                        uid = obj.UID()
                        transobj.getField(field.getName()).getMutator(transobj)(uid)
        ...
        # Check if the language of the uploaded file is the same as the current object's
        # If not, then a file with a 'wrong' language was uploaded into the translated CallForContractors
        ...
        # Tried to setLanguage at this point, but I get an error in manage_cutObjects
        # (_p_jar seems to be None at this point
        # therefore, setting language is done im handle_edit_begun and in CallsHelper::checkForNewUploads
...
def handle_object_edited(obj, event):
    """ If the canonical object was edited, check if we need to update references in the translations """
    if obj.isCanonical():
        fields = [x for x in obj.Schema().fields() if isinstance(x, ReferenceField)]
        # only handle fields which are not language independent - the others are already taken care of
        # by LinguaPlone
        ...
        for field in fields:
            ...
            for tlang in translations.keys():
                ...
                if not tfield.getRaw(translations[tlang][0]):
                    ...
                    # the correct language version might already have been moved to the target
                    # so look on the target first and then on the canonical
                    refobjtrans = getattr(translations[tlang][0], transname, getattr(obj, transname, None))
                    if not refobjtrans:
                        # the language suffix might be uppercase on the file...
...
def handle_object_wf_modified(obj, event):
    """ If a Call's workflow state is modified, also modify all objects within it accordingly 
    """
